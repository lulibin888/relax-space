...
export function BreathingGuide() {
    ...
    timerRef.current = setInterval(() => {
      setPhaseTime(prev => {
        ...
        if (prev + 1 >= config.duration) {
          ...
          setCurrentPhase(pattern[nextIndex]);
          playChimeSound();
  ...
  const getPhaseGradient = () => {
    switch (currentPhase) {
      case 'inhale': return 'oklch(0.78 0.10 240)';
      case 'hold': return 'oklch(0.75 0.08 260)';
      case 'exhale': return 'oklch(0.78 0.08 200)';
      case 'rest': return 'oklch(0.80 0.06 180)';
      default: return 'oklch(0.82 0.04 240)';
    }
  };
  ...
  return (
    <div className="flex flex-col items-center gap-5 w-full">
          ...
          <button
            ...
            style={{
              background: JSON.stringify(pattern) === JSON.stringify([...p.value])
                ? 'linear-gradient(135deg, oklch(0.85 0.06 240), oklch(0.82 0.05 220))'
                : 'oklch(0.97 0.008 240)',
              color: JSON.stringify(pattern) === JSON.stringify([...p.value])
                ? 'oklch(0.35 0.06 240)' : 'oklch(0.55 0.03 240)',
              boxShadow: JSON.stringify(pattern) === JSON.stringify([...p.value])
                ? '0 2px 8px oklch(0.72 0.08 240 / 0.2)' : 'none',
      ...
      <div className="relative flex items-center justify-center w-52 h-52">
        ...
        {isRunning && (
          <div
            ...
            style={{
              border: `1.5px solid ${getPhaseGradient()} / 0.2`,
        ...
        <div
          ...
          style={{
            ...
            background: isRunning
              ? `radial-gradient(circle at 40% 35%, ${getPhaseGradient()} / 0.25, ${getPhaseGradient()} / 0.1)`
              : 'radial-gradient(circle, oklch(0.93 0.03 240 / 0.25), oklch(0.90 0.02 240 / 0.08))',
            transitionDuration: currentPhase === 'inhale' ? '4000ms' : currentPhase === 'exhale' ? '6000ms' : '500ms',
            transitionTimingFunction: 'cubic-bezier(0.4, 0, 0.2, 1)',
            boxShadow: isRunning
              ? `0 0 50px ${getPhaseGradient()} / 0.2, inset 0 0 20px oklch(0.92 0.03 240 / 0.2)`
              : '0 0 20px oklch(0.88 0.02 240 / 0.1)',
            border: isRunning ? `1.5px solid ${getPhaseGradient()} / 0.25` : '1.5px solid oklch(0.92 0.02 240 / 0.25)',
          }}
        >
          <div className="text-center">
            {isRunning && phaseConfig ? (
              <>
                <p className="text-2xl font-light" style={{ color: 'oklch(0.35 0.06 240)' }}>{phaseConfig.label}</p>
                <p className="text-sm mt-1 font-light" style={{ color: 'oklch(0.55 0.05 240)' }}>{phaseConfig.duration - phaseTime}</p>
              </>
            ) : (
              <p className="text-base font-light" style={{ color: 'oklch(0.55 0.04 240)' }}>准备好了吗</p>
            )}
          </div>
        </div>
      </div>
      ...
      {isRunning && phaseConfig && (
        <p className="text-xs text-center" style={{ color: 'oklch(0.55 0.04 240)', animation: 'fadeIn 0.5s ease-out' }}>
          {phaseConfig.instruction}
        </p>
      )}

      {cycleCount > 0 && (
        <p className="text-[11px]" style={{ color: 'oklch(0.60 0.04 240)' }}>已完成 {cycleCount} 个循环</p>
      )}
      ...
      <button
        ...
        style={{
          background: isRunning
            ? 'linear-gradient(135deg, oklch(0.62 0.08 10), oklch(0.58 0.06 10))'
            : 'linear-gradient(135deg, oklch(0.65 0.12 240), oklch(0.62 0.10 220))',
          boxShadow: isRunning
            ? '0 4px 15px oklch(0.62 0.08 10 / 0.3)'
            : '0 4px 15px oklch(0.65 0.10 240 / 0.3)',
        }}
      >
        {isRunning ? '结束' : '开始呼吸'}
      </button>
