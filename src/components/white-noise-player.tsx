...
const NOISE_TYPES: { type: NoiseType; label: string; emoji: string; color: string }[] = [
  { type: 'white', label: '白噪音', emoji: '🌫️', color: 'oklch(0.70 0.04 240)' },
  { type: 'pink', label: '粉噪音', emoji: '🌸', color: 'oklch(0.72 0.08 340)' },
  { type: 'brown', label: '棕噪音', emoji: '🍂', color: 'oklch(0.62 0.08 60)' },
  { type: 'rain', label: '雨声', emoji: '🌧️', color: 'oklch(0.62 0.10 240)' },
    ...
    <div className="flex flex-col items-center gap-5 w-full">
        ...
        {NOISE_TYPES.map(({ type, label, emoji, color }) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-center gap-1.5 p-4 rounded-2xl transition-all duration-300 cursor-pointer"
              style={{
                background: isActive
                  ? `linear-gradient(145deg, ${color} / 0.12, ${color} / 0.06)`
                  : 'oklch(0.98 0.005 240)',
                boxShadow: isActive
                  ? `0 4px 16px ${color} / 0.2, inset 0 1px 0 oklch(1 0 0 / 0.3)`
                  : '0 1px 3px oklch(0.90 0 0 / 0.15)',
                transform: isActive ? 'translateY(-2px)' : 'translateY(0)',
                border: isActive ? `1px solid ${color} / 0.25` : '1px solid oklch(0.95 0.01 240 / 0.4)',
              }}
            >
              <span className="text-2xl transition-transform duration-300" style={{ transform: isActive ? 'scale(1.15)' : 'scale(1)' }}>
                {emoji}
              </span>
              <span className="text-[11px] font-medium" style={{ color: isActive ? 'oklch(0.35 0.04 240)' : 'oklch(0.55 0.03 240)' }}>
                {label}
              </span>
              {isActive && (
      ...
      <div className="w-full max-w-xs flex items-center gap-3 px-2">
        ...
        <input
          ...
          style={{ background: `linear-gradient(to right, oklch(0.68 0.10 240) ${volume * 100}%, oklch(0.93 0.01 240) ${volume * 100}%)` }}
        />
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="oklch(0.55 0.03 240)" strokeWidth="2">
          <path d="M11 5L6 9H2v6h4l5 4V5z" />
          <path d="M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07" />
        </svg>
        <span className="text-[11px] w-7 text-right" style={{ color: 'oklch(0.55 0.03 240)' }}>
